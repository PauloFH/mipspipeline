  ...
  vector<string> readDocument(const std::string &filename)
{
    ...
    sc_uint<32> binaryValue = 0;
     map<string, int> opcodeValues = {
        ...
        {"cmp", 0b0101},{"add", 0b0110}, {"addi", 0b0111}, {"sub", 0b1000}, {"lw", 0b1001},
        {"sw", 0b1010},{"j", 0b1011}, {"beq", 0b1100},{"bne", 0b1101}
...
sc_uint<32> registradorToBinary(sc_uint<4> opcd, string reg, int tp){
   std::unordered_map<std::string, sc_uint<6>> registradores = {
        {"$zero", 0b000000},{"$at", 0b000001}, {"$v0", 0b000010}, {"$v1", 0b000011},
        {"$a0", 0b000100}, {"$a1", 0b000101}, {"$a2", 0b000110}, {"$a3", 0b000111},
        {"$t0", 0b001000}, {"$t1", 0b001001}, {"$t2", 0b001010}, {"$t3", 0b001011},
        {"$t4", 0b001100},{"$t5", 0b001101},{"$t6", 0b001110},{"$t7", 0b001111},
    ...
    if(reg == "$zero"){
        ...
        if(tp == 1){
            ...
            cout << endl << "no lw/sw: " <<endl<<" registrador destino/origem: "<< registerIntern<< " em biset: "<<bitset<6>(registerIntern) <<endl;
